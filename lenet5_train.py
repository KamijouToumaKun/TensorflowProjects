...
def train(mnist):
    # 写明BATCH_SIZE，而不是写成NONE；应该是因为维度不止二维
    X = tf.placeholder(tf.float32, [BATCH_SIZE, lenet5_interface.IMAGE_SIZE, lenet5_interface.IMAGE_SIZE, lenet5_interface.NUM_CHANNELS], name='x-input')
    ...
    # 不使用滑动平均类的输出
    out = lenet5_interface.interface(X, True, regularizer)
    ...
    # avg_out = lenet5_interface.interface(X, True, regularizer)
    ...
    with tf.Session() as sess:
        ...
        for i in range(TRAINING_EPOCH):
            ...
            # 这里需要将xs给reshape
            reshaped_xs = np.reshape(xs, (BATCH_SIZE, lenet5_interface.IMAGE_SIZE, lenet5_interface.IMAGE_SIZE, lenet5_interface.NUM_CHANNELS))
            sess.run(train_op, feed_dict={X:reshaped_xs, Y:ys})
