...
def train(mydata):
    ...
    # 不使用滑动平均类的输出
    out = mydataset_interface.interface(X, regularizer)
    ...
    # 使用滑动平均类的输出，现在放到mydataset_eval中定义：带regularizer参数的interface只调用一次
    # avg_out = mydataset_interface.interface(X, regularizer)
    ...
    learning_rate = tf.train.exponential_decay(
        ...
        epoch, # 当前迭代的轮数
        len(mydata.img) / BATCH_SIZE, # 过完所有的训练数据需要的迭代轮数
        # mydata.train.num_examples / BATCH_SIZE, # 过完所有的训练数据需要的迭代轮数
    ...
    with tf.Session() as sess:
        ...
        for i in range(TRAINING_EPOCH):
            ...
            xs, ys = mydata.next_batch(BATCH_SIZE)
            sess.run(train_op, feed_dict={X:xs, Y:ys})

def main(argv=None):
    mytraindata = mydataset(['10'], ['画', '真人'])
    train(mytraindata)
